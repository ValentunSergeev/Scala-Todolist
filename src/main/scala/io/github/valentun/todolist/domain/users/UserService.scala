...
class UserService[F[_]](userRepo: UserRepositoryAlgebra[F], validation: UserValidationAlgebra[F]) {
  ...
  def update(user: User, requesterId: Long)(implicit M: Monad[F]): EitherT[F, UserError, User] =
    for {
      _ <- validation.exists(user.id)
      _ <- validation.owns(user.id.get, requesterId)
      saved <- userRepo.update(user).toRight[UserError](UserNotFoundError)
    } yield saved
